...
* `부분 점수` - 제출한 프로그램이 출력한 답이 최적의 답이 아니더라도 부분 점수가 주어지는 문제입니다.
* `포인트 경쟁` - 점수가 다른 참가자와의 상대평가로 확정되는 문제입니다. 다른 참가자가 답안을 제출함에 의해 내 점수가 변할 수 있습니다. 자세한 채점 규칙은 문제를 참고하세요. 모든 포인트 경쟁 문제는 자동으로 부분 점수가 주어집니다.
...
예선 진행 도중, 각 스테이지별 페이지 등을 통해 지금까지 획득한 점수를 확인할 수 있습니다. 단, `포인트 경쟁` 문제들은 점수가 바뀔 수 있습니다.
