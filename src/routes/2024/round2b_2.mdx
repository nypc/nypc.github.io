...
<div style={{ display: "flex" }}>
  <Figure src="/2024/6-1.76db021f1a981900.png" width={200} />
</div>
...
<div style={{ display: "flex" }}>
  <Figure src="/2024/6-2.1b15836d3518c498.png" width={500} />
</div>
...
<div style={{ display: "flex" }}>
  <Figure src="/2024/6-3.3821adb4d54fbf64.png" width={500} />
</div>
...
DP를 사용해서 문제를 풀 수 있다. $D_{i, j}$를 $i$ 번째 직사각형을 마지막 직사각형으로, $j$ 번째 직사각형을 마지막 바로 앞의 직사각형으로 하는 계단의 수라고 하자. 여기서 당연히 $j$는 $i$보다 크지 않다.
...
해당 계산을 나이브하게 할 경우, $\mathcal{O} \left( N^3 \right)$의 시간복잡도를 가지므로 만점을 받을 수 없다. 이분탐색이나 투 포인터 등을 활용하여 최적화하면 각 $D_{i, j}$를 $\mathcal{O} \left( \lg N \right)$ 또는 $\mathcal{O} \left( 1 \right)$의 시간복잡도에 구할 수 있다. 이 경우, 총 $\mathcal{O} \left( N^2 \lg N \right)$ 또는 $\mathcal{O} \left( N^2 \right)$의 시간복잡도를 가지고, 만점을 받을 수 있다.
