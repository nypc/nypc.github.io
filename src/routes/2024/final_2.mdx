...
<div style={{ display: "flex" }}>
  <Figure src="/2024/final-3-1.baa74e9c954d20b2.png" width={550} />
</div>
...
답은 항상 $T$ 또는 $T'$의 첫 몇 글자(접두사)이다. 따라서 $T$ 또는 $T'$의 길이가 $K, K-1, \cdots, 1$인 접두사만 확인하면 되고, $P$의 후보가 주어졌을 때 $P$와 $P'$로 $T$를 덮을 수 있는지는 $\mathcal{O}\left(NK\right)$ 시간복잡도로 확인할 수 있으므로 전체 문제를 $\mathcal{O}\left(NK^2\right)$ 시간복잡도로 해결할 수 있다.

$K \le 20$이므로 $T$의 길이가 $1, 2, \cdots, K$인 부분문자열을 `int` 타입의 정수 하나로 표현할 수 있음을 이용하면 $\mathcal{O}\left(NK\right)$ 시간복잡도로도 해결할 수 있다.
