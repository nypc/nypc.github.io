...
<div style={{ display: "flex" }}>
  <Figure src="/2024/final-1-1.51c664e5f7c88674.png" width={800} />
</div>
...
"팀 점수차"를 "레드팀의 점수"에서 "블루팀의 점수"를 뺀 부호 있는 값이라고 하자.\ 이제, 다음과 같은 DP를 생각하자.

$D_i :=$ 첫 $i$ 개의 라운드가 진행되었을 때 팀 점수차로 가능한 값들의 집합.\ $E_i :=$ 마지막에서 $i$ 번째 라운드가 진행되었을 때 팀 점수차로 가능한 값들의 집합.

$D_0 := \{ 0 \}$, $E_0 := \{ \pm K \}$로 정의하자. 다음과 같은 점화식을 생각할 수 있다: $$ D*i = \left( D*{i-1} + V_i \right) \cap [-K+1, K-1] $$

여기서, $S_i = \texttt{R}$이면 $V_i = \{ 1 \}$, $S_i = \texttt{B}$이면 $V_i = \{ -1 \}$, $S_i = \texttt{?}$이면 $V_i = \{ \pm 1 \}$이다. $E_i$에 대해서도 유사한 형태의 점화식을 얻는다.

유실된 $i$번째 라운드에 레드팀이 승리하는 것이 가능한지 여부는 $\left( D_{i-1} + 1 \right) \cap E_{N-i} \ne \emptyset$과 동치이다. 블루팀이 승리하는 것이 가능한지 여부 또한 유사하게 알 수 있다. 이제, 유실된 라운드에서 한 팀만 승리할 수 있었음을 안다면, 그 라운드의 기록을 복원할 수 있다.
