...
<Solution>
배열 $dp[N][M]$를 이용하여 풀되, 좌에서 우로 sweep하면서 각 값을 채워나간다.

배열 $dp[N][M]$에 있는 각 값은 해당 격자를 청소하거나 또는 지나가거나 하는 로봇의 번호를 말한다. 
로봇 번호는 $1$부터 하나씩 증가한다.

입력 배열 $val[r][c]$엔 $X$가 마크되어 있으면 $1$, 그렇지 않으면 $0$이다.
