...
<Solution>
먼저, 칸의 번호가 주어지면 그 칸의 위치를 효율적으로 찾는 방법을 생각하자. 
격자판의 테두리에서 $K$칸 이내로 떨어진 칸들의 번호는 연속한 구간을 이룬다.
따라서, $K$에 대한 이분 탐색을 통해 찾고자 하는 칸이 테두리에서 얼마나 떨어져 있는지를 알아낼 수 있다. 
이후에는 간단한 케이스 처리를 통해 칸의 좌표를 알아낼 수 있다.

이를 구현하면 각 쿼리에 대해 $O(log N)$에 처리할 수 있다. 
$K$에 대한 이차 방정식을 세워 쿼리 당 $O(1)$에도 해결할 수 있다.
</Solution>
