...
<Solution>
수열에서 각 수가 들어온 횟수를 배열에 저장하자. 
수열이 조건을 만족하기 위해서는 각 수가 한번 이하로 등장해야 하므로, 배열에 저장된 값이 $1$을 넘으면 안된다.
이러한 방식으로 부분수열의 왼쪽 끝점을 고정했을 때, 가능한 오른쪽 끝점의 최댓값을 구할 수 있다.
한편 왼쪽 끝점이 점점 증가할 때 가능한 오른쪽 끝점의 최댓값 또한 점점 증가한다는 것을 알 수 있다.
이 성질을 활용하면 $O(N)$에 문제를 해결할 수 있다. 
이러한 테크닉을 two pointer 또는 sliding window라 한다.
</Solution>
